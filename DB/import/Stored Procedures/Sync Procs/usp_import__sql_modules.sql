...
BEGIN;
	...
	*/
	...
	WHERE x.DatabaseID = @DatabaseID
		...
		AND NOT EXISTS (SELECT * FROM @output d WHERE d.ObjectID = x.ObjectID)
		AND x.IsDeleted = 0;
	...
	SET   x._ModifyDate				= SYSUTCDATETIME()
		...
		, x._ObjectDefinitionID		= od.ObjectDefinitionID
	...
	FROM dbo._sql_modules x
		JOIN @output o ON o.ObjectID = x._ObjectID
		JOIN #Dataset d ON d.ID = o.ID
		JOIN dbo.ObjectDefinition od ON od.ObjectDefinitionHash = d._ObjectDefinitionHash
	WHERE x._RowHash <> d._RowHash OR x._ObjectDefinitionID <> od.ObjectDefinitionID;
	...
	SELECT @DatabaseID, y.ObjectID, d._RowHash
		, d.[object_id], od.ObjectDefinitionID, d.uses_ansi_nulls, d.uses_quoted_identifier, d.is_schema_bound, d.uses_database_collation, d.is_recompiled, d.null_on_null_input, d.execute_as_principal_id, d.uses_native_compilation, d.inline_type, d.is_inlineable
	FROM #Dataset d
