    ...
    BEGIN TRY
        ...
        BEGIN TRAN;
            ...
            SET   x._DatabaseID                                = sd._DatabaseID
                ...
                , x.is_data_lake_replication_enabled           = d.is_data_lake_replication_enabled
                , x.is_event_stream_enabled                    = d.is_event_stream_enabled
                , x.data_compaction                            = d.data_compaction
                , x.data_compaction_desc                       = d.data_compaction_desc
                , x.data_lake_log_publishing                   = d.data_lake_log_publishing
                , x.data_lake_log_publishing_desc              = d.data_lake_log_publishing_desc
                , x.is_vorder_enabled                          = d.is_vorder_enabled
                , x.is_proactive_statistics_refresh_on         = d.is_proactive_statistics_refresh_on
                , x.is_optimized_locking_on                    = d.is_optimized_locking_on
            ...
            FROM @Dataset d
                LEFT JOIN dbo.[Database] sd ON sd._InstanceID = @InstanceID AND sd.DatabaseName = d.[name]
            WHERE NOT EXISTS (SELECT * FROM dbo._databases x WHERE x._InstanceID = @InstanceID AND x.[name] = d.[name]);
