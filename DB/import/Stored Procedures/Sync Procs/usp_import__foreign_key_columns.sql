...
BEGIN;
	...
	SET   x._ParentObjectID			= p.ObjectID
		...
		, x._ReferencedColumnID		= r.ColumnID
	...
	SELECT @DatabaseID, y.ObjectID, p.ObjectID, p.ColumnID, r.ObjectID, r.ColumnID, d._RowHash
	...
	WHERE NOT EXISTS (
			SELECT *
			FROM dbo._foreign_key_columns x
			WHERE x._ObjectID  = y.ObjectID AND x.constraint_column_id = d.constraint_column_id
		);
