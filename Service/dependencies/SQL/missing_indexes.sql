...
FROM (
    SELECT _SchemaName = n.SchemaName
        ...
        , _ObjectType = o.[type]
        --
        , missing_index_hash = CONVERT(binary(32), HASHBYTES('SHA2_256', c.column_data))
        ...
        */
        CROSS APPLY (
            SELECT n.SchemaName, n.ObjectName -- Not needed here for storing the column data, but helps with making the missing_index_hash more unique
                , [EQUALITY]   = JSON_QUERY((SELECT '[' + STRING_AGG(IIF(column_usage = 'EQUALITY'  , x.col_name_clean, NULL), ',') WITHIN GROUP (ORDER BY column_name) + ']'))
                , [INEQUALITY] = JSON_QUERY((SELECT '[' + STRING_AGG(IIF(column_usage = 'INEQUALITY', x.col_name_clean, NULL), ',') WITHIN GROUP (ORDER BY column_name) + ']'))
                , [INCLUDE]    = JSON_QUERY((SELECT '[' + STRING_AGG(IIF(column_usage = 'INCLUDE'   , x.col_name_clean, NULL), ',') WITHIN GROUP (ORDER BY column_name) + ']'))
